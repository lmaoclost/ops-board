...
test("novo projeto com nota exibe nota; renomear edita nota", async ({ page }) => {
  await page.goto("/");
...
test("bloquear tarefa pede motivo opcional e mostra no badge", async ({ page }) => {
  ...
  await page.getByLabel("motivo do bloqueio (opcional)").fill("sem acesso ao banco");
  await page.getByRole("button", { name: "salvar" }).click();
...
test("seções reordenam arrastando pelo grip", async ({ page }) => {
  ...
  await drag(page, page.getByLabel("arrastar seção p/ reordenar").nth(1), headers.nth(0));
