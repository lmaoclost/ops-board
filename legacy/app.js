...
function load() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && Array.isArray(parsed.projetos) && parsed.projetos.length)
        loaded = parsed;
    }
  } catch (e) {
    /* vazio */
...
function save() {
  ...
  if (el) {
    ...
    el.classList.add("text-fired");
    setTimeout(() => el.classList.remove("text-fired"), 900);
  }
}
...
function findProject(pid) {
  return state.projetos.find((p) => p.id === pid);
}
function findSection(pid, sid) {
  return findProject(pid)?.sections.find((s) => s.id === sid);
}
function findTask(pid, sid, tid) {
  return findSection(pid, sid)?.tasks.find((t) => t.id === tid);
}
...
function renderStats() {
  ...
  const doneToday = allTasks().filter(
    (t) => t.doneAt && t.doneAt.slice(0, 10) === todayISO(),
  ).length;
  $("#stats").innerHTML =
...
function renderBoard() {
  ...
  board.innerHTML = visibleProjs
    .map((p) => renderProject(p, filtrado))
    .join("");
}

function renderProject(p, filtrado) {
  const visibleSecs = p.sections.filter(
    (s) =>
      !filtrado ||
      s.tasks.some(matchTask) ||
      (ui.query &&
  ...
  return `<section class="proj fade-in" data-pid="${p.id}">
    <div class="proj-header">
      ...
      <span class="ml-auto flex items-center gap-1">
        ...
        <button class="iconbtn" data-action="proj-rename" title="editar projeto">✎</button>
...
function renderSection(pid, s, filtrado) {
  ...
  return `<div class="sec ${open ? "open" : ""}" data-pid="${pid}" data-sid="${s.id}">
    ...
    <div class="sec-body">
      ...
      ${
        open
          ? `<div class="sec-tasks" data-dropzone="tasks">
...
function dueHtml(t) {
  ...
  const soon =
    t.status !== "done" &&
    !overdue &&
    new Date(t.due + "T00:00:00") - new Date(todayISO() + "T00:00:00") <=
      2 * 86400000;
  if (overdue)
    return `<span class="tag tag-overdue" title="vencimento ${esc(t.due)}">${fmtDate(t.due)} vencida</span>`;
...
function renderTask(pid, sid, t) {
  ...
  const statusOpts = STATUS_ORDER.map(
    (k) =>
      `<option value="${k}" ${k === t.status ? "selected" : ""}>${STATUS[k].label}</option>`,
  ).join("");
...
function renderKanban() {
  ...
  for (const c of cols)
    if (ui.prioSort) c.items.sort((a, b) => a.t.prio - b.t.prio);
...
function handleKanbanDrop(e) {
  ...
  if (t) {
    ...
    const rect =
      !wasDone && status === "done" && card && card.querySelector(".led")
        ? card.querySelector(".led").getBoundingClientRect()
        : null;
...
function deleteSection(pid, sid) {
  findProject(pid).sections = findProject(pid).sections.filter(
    (s) => s.id !== sid,
  );
  mutation();
}
...
function modal({ title, fields, submitLabel = "salvar", onOpen }) {
  return new Promise((resolve) => {
    ...
    const inputHtml = (f) => {
      ...
      if (f.type === "select")
        return `<select class="input-line w-full" data-k="${f.key}">${(
          f.options || []
        )
          .map(
            (o) =>
    ...
    $("[data-elm=backdrop]", root).addEventListener("mousedown", (e) => {
...
function burstConfetti(x, y) {
  ...
  for (let i = 0; i < bits; i++) {
    ...
    s.style.setProperty(
      "--dx",
      ((Math.random() - 0.5) * 150).toFixed(1) + "px",
    );
    s.style.setProperty(
      "--dy",
      (H + 60 + Math.random() * 100).toFixed(1) + "px",
    );
    s.style.setProperty(
      "--rot",
      (Math.random() * 200 - 100).toFixed(1) + "deg",
    );
...
function openProjectModal(p) {
  modal({
    ...
    fields: [
      ...
      {
        key: "blocked",
        label: "marcar como stuck / bloqueado",
        value: p.blocked,
        type: "checkbox",
      },
    ],
...
function openSectionModal(pid, s) {
  modal({
    ...
    if (v && v.title.trim()) renameSection(pid, s.id, v.title.trim());
  });
}
...
function openTaskModal(pid, sid, t) {
  modal({
    ...
    fields: [
      ...
      {
        key: "blocked",
        label: "marcar como bloqueada / stuck",
        value: t.blocked,
        type: "checkbox",
      },
    ],
...
function dndSerialize(src) {
  return JSON.stringify({ pid: src.pid, sid: src.sid, tid: src.tid });
}
...
function handleDragStart(e) {
  ...
  if (!task) return;
  dndState.src = {
    pid: task.dataset.pid,
    sid: task.dataset.sid,
    tid: task.dataset.tid,
  };
...
function clearDndFx() {
  $$(
    ".task.dragging, .task.drop-top, .task.drop-bottom, .kan-card.dragging, .kan-col.drop-zone, .dock-cell.drag-over",
  ).forEach((n) =>
    n.classList.remove(
      "dragging",
      "drop-top",
...
function handleTaskDragOver(e) {
  ...
  e.dataTransfer.dropEffect = "move";
  zone
    .querySelectorAll(".drop-top, .drop-bottom")
    .forEach((n) => n.classList.remove("drop-top", "drop-bottom"));
  const over = e.target.closest(".task");
  if (!over) {
    zone.lastElementChild && zone.lastElementChild.classList.add("drop-bottom");
    return;
  }
  const r = over.getBoundingClientRect();
  over.classList.add(
    e.clientY < r.top + r.height / 2 ? "drop-top" : "drop-bottom",
  );
}

function handleDropOnTasks(e) {
  const zone = e.target.closest('[data-dropzone="tasks"]');
  if (!zone || !dndState.src) {
    clearDndFx();
    return;
  }
  e.preventDefault();
  ...
  const insert = posEl
    ? destSec.tasks.findIndex((t) => t.id === posEl.dataset.tid)
    : -1;
  const targetIdx =
    posEl && posEl.classList.contains("drop-top")
      ? insert
      : insert === -1
        ? destSec.tasks.length
        : insert + 1;
...
function handleDropOnDock(e) {
  ...
  if (t) {
    ...
    const row = document.querySelector(
      `[data-pid="${src.pid}"][data-sid="${src.sid}"][data-tid="${src.tid}"]`,
    );
    const rect =
      !wasDone && status === "done" && row
        ? row.querySelector(".led").getBoundingClientRect()
        : null;
    applyStatus(t, status);
    mutation();
    if (rect) burstAt(rect.left + rect.width / 2, rect.top + rect.height / 2);
  ...
  switch (act) {
    case "proj-add-sec":
      modal({
        ...
        if (v && v.title.trim()) addSection(pid(), v.title.trim());
      });
      break;
    case "proj-rename":
      openProjectModal(findProject(pid()));
      break;
    case "proj-del":
      const p = findProject(pid());
    ...
    case "sec-toggle":
      ...
      break;
    case "sec-del":
      if (confirmBox("excluir seção e todas as tarefas dela?"))
        deleteSection(pid(), sid());
      break;
    case "sec-rename":
      openSectionModal(pid(), findSection(pid(), sid()));
      break;
    case "task-toggle": {
      const t = findTask(pid(), sid(), tid());
      const wasDone = t.status === "done";
      const rect =
        t.status === "done"
          ? null
          : el.closest(".task").getBoundingClientRect();
    ...
    case "task-prio":
      setTaskPrio(
        ...
        sid(),
        tid(),
        { 1: 2, 2: 3, 3: 1 }[findTask(pid(), sid(), tid()).prio] || 3,
      );
      break;
    case "task-edit":
      openTaskModal(pid(), sid(), findTask(pid(), sid(), tid()));
      break;
    case "task-del":
...
function openNewProject() {
  modal({
    ...
    if (v && v.title.trim()) addProject(v.title.trim());
  });
}
...
function handleBoardChange(e) {
  ...
  if (el.matches('[data-action="task-status"]')) {
    ...
    const wasDone = t.status === "done";
    const rect =
      el.value === "done" && !wasDone ? row.getBoundingClientRect() : null;
...
function bindBoardDnD() {
  ...
  const dropableDragOver = (e) => {
    ...
    if (e.target.closest('[data-dropzone="tasks"]'))
      return handleTaskDragOver(e);
...
function bindTopbar() {
  ...
  $("#view-btn").addEventListener("click", () =>
    setView(ui.view === "list" ? "kanban" : "list"),
  );
  ...
  $("#btn-reset").addEventListener("click", () => {
    if (confirmBox("apagar todos os dados?")) {
...
function shortcutHelp() {
  toast(
    "p novo projeto · s nova seção · n nova tarefa · 1-5 filtros · k kanban · t tema · ? ajuda · esc limpa",
  );
}
